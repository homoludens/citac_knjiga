...
the API 35 x86_64 emulator. The host-only report generated by `qualify.py`
remains `no-pass` because it cannot execute Android evidence; it must not
replace the checked-in promoted report.

The checked-in report is a promoted pass for the defined matrix: production is
API 33 `arm64-v8a`, development is API 35 `x86_64`, and both targets passed the
candidate suite. API 30 and API 36 are explicitly non-gating and not executed.
The exact candidate is selected in `document-pdf` only for this matrix.
